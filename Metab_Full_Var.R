# Gunzip the INTERVAL_MRC-Epi_M00053_sorted_chr_1.tbl.gz file
library(R.utils)
gunzip("Reverse_MR/INTERVAL_MRC-Epi_M00053_sorted_chr_1.tbl.gz")
# Read the INTERVAL_MRC-Epi_M00053_sorted_chr_1.tbl file
try <- readr::read_tsv("Reverse_MR/INTERVAL_MRC-Epi_M00053_sorted_chr_1.tbl")
...
for (i in 1:length(file_names)) {
  ...
  matched_ivs$proxy <- NA
  matched_ivs$proxy_EAF <- NA
  matched_ivs$proxy_R2 <- NA
  matched_ivs$proxy_EffectAllele <- NA
  matched_ivs$proxy_NonEffectAllele <- NA
  ...
  for (k in 1:nrow(matched_ivs)) {
    ...
    if (!match_flag) {
      ...
      if (nrow(ld_proxies) > 0 ) {
        ...
        ld_proxies$reverseMarkerName <- paste0("chr", ld_proxies$CHR_B, ":", ld_proxies$BP_B, ":", ld_proxies$B2, ":", ld_proxies$B1)
        for (j in 1:nrow(ld_proxies)) {
          if (ld_proxies$MarkerName[j] %in% combined_data$MarkerName) {
            # Select the row from combined_data where the MarkerName matches the IV
            out_values <- combined_data[combined_data$MarkerName == ld_proxies$MarkerName[j], ]
            matched_ivs$proxy[k] <- ld_proxies$SNP_B[j]
            matched_ivs$proxy_EAF[k] <- ld_proxies$MAF_B[j]
            matched_ivs$proxy_R2[k] <- ld_proxies$R[j]
            matched_ivs$proxy_EffectAllele[k] <- ld_proxies$B1[j]
            matched_ivs$proxy_NonEffectAllele[k] <- ld_proxies$B2[j]
            matched_ivs$out_Chromosome[k] <- out_values$chrom
            matched_ivs$out_Position[k] <- out_values$chromStart
            matched_ivs$out_EffectAllele[k] <- out_values$Allele1
            matched_ivs$out_NonEffectAllele[k] <- out_values$Allele2
            ...
            matched_ivs$out_Pval[k] <- out_values$Pvalue
            proxy_flag <- TRUE
            T2DM_need_proxy <- T2DM_need_proxy + 1
            break
          } else if (ld_proxies$reverseMarkerName[j] %in% combined_data$MarkerName) {
            # Select the row from combined_data where the reverseMarkerName matches the IV
            out_values <- combined_data[combined_data$MarkerName == ld_proxies$reverseMarkerName[j], ]
            matched_ivs$proxy[k] <- ld_proxies$SNP_B[j]
            matched_ivs$proxy_EAF[k] <- ld_proxies$MAF_B[j]
            matched_ivs$proxy_R2[k] <- ld_proxies$R[j]
            matched_ivs$proxy_EffectAllele[k] <- ld_proxies$B2[j]
            matched_ivs$proxy_NonEffectAllele[k] <- ld_proxies$B1[j]
            matched_ivs$out_Chromosome[k] <- out_values$chrom
            matched_ivs$out_Position[k] <- out_values$chromStart
            matched_ivs$out_EffectAllele[k] <- out_values$Allele1
            matched_ivs$out_NonEffectAllele[k] <- out_values$Allele2
    ...
    # If the match_flag and proxy_flag are still FALSE, remove the IV
    if ((match_flag == FALSE) & (proxy_flag == FALSE)) {
      ...
      if (nrow(ld_proxies) > 0 ) {
        ...
        for (j in 1:nrow(ld_proxies)) {
          if (ld_proxies$MarkerName[j] %in% combined_data$MarkerName) {
            ...
            matched_ivs$out_Pval[k] <- out_values$Pvalue
            proxy_flag <- TRUE
            HBA1C_need_proxy <- HBA1C_need_proxy + 1
            break
          } else if (ld_proxies$reverseMarkerName[j] %in% combined_data$MarkerName) {
            # Select the row from combined_data where the reverseMarkerName matches the IV
            out_values <- combined_data[combined_data$MarkerName == ld_proxies$reverseMarkerName[j], ]
  ...
  # Add the number of Proxies and Removed IVs to compid_IVs_list
  compid_IVs_list <- c(compid_IVs_list, T2DM_need_proxy, T2DM_removed_IVs, FG_need_proxy, FG_removed_IVs, 
                       HBA1C_need_proxy, HBA1C_removed_IVs)
  ...
  print(paste("T2DM Proxies:", T2DM_need_proxy))
  print(paste("FG Proxies:", FG_need_proxy))
  print(paste("HbA1c Proxies:", HBA1C_need_proxy))
...
# Check the values of the chromosome_counter_list
# Should all be 22
chromosome_counter_list
# Check the values of the compid_IVs_list
# Check the number of IVs for each metabolite and the number of proxies and removed IVs
compid_IVs_list

# 316, 374, 439
# 7 minutes for each metabolite -- guess 7x70 = 490 minutes = 8 hours
