...
MR_simple_median_out <- MendelianRandomization::mr_median(MRObject, weighting = "simple")
...
results_df <- data.frame(
  ...
  Simple_Median_Estimate = rep(NA, number_of_metabolites),
  Simple_Median_SE = rep(NA, number_of_metabolites),
  Simple_Median_Pval = rep(NA, number_of_metabolites),
...
for (metabolite_file in metabolite_files) {
  ...
  if (nrow(metabolite_data) > 2) {
    ...
    # Store the results
    results_df$Simple_Median_Estimate[metabolite_counter] <- MR_simple_median_out$Estimate
    results_df$Simple_Median_SE[metabolite_counter] <- MR_simple_median_out$StdError
    results_df$Simple_Median_Pval[metabolite_counter] <- MR_simple_median_out$Pvalue
