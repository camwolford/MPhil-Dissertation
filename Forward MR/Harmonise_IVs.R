...
for (metabolite_file in metabolite_files) {
  ...
  tolerance = 0.08 # This is the same as is used in the TwoSampleMR package. It was handled separately here due to how proxy data was stored.
    ...
    if (!is.na(metabolite_data$proxy[i])) {
      ...
      if (metabolite_data$proxy_EAF[i] < lower_tolerance | metabolite_data$proxy_EAF[i] > upper_tolerance) {
        ...
        if (metabolite_data$proxy_EAF[i] > lower_inverse_tolerance & metabolite_data$proxy_EAF[i] < upper_inverse_tolerance) {
          ...
          metabolite_data$SNP[i] <- NA
          exposure_data$SNP[i] <- NA
          outcome_data$SNP[i] <- NA
  ...
  # Remove rows with NAs in the SNP column
  metabolite_data <- metabolite_data[!is.na(metabolite_data$SNP), ]
  exposure_data <- exposure_data[!is.na(exposure_data$SNP), ]
  outcome_data <- outcome_data[!is.na(outcome_data$SNP), ]
  # Rename the columns
